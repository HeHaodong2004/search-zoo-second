...
class PathPlanningMaskEnv(gym.Env):
    ...
    def __init__(
        ...
        # world collision checker still for precise masks
    ...
    def _get_obs(self) -> Dict:
        ...
        # --- laser scan on inflated grid ---
        scans = np.zeros(self.n_lasers, dtype=np.float32)
        ...
        for idx, ang in enumerate(self.laser_angles):
            ...
            for s in range(max_steps):
                ...
                if i<0 or i>=H or j<0 or j>=W or self.inflated[i,j]:
                    scans[idx] = min(self.max_scan_dist, s*self.resolution)
                    break
        ...
        return {'scan': scans, 'goal_vec': goal_vec, 'action_mask': self.action_masks()}
