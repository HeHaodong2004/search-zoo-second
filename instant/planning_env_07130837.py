...
class PathPlanningMaskEnv(gym.Env):
    ...
    def __init__(
        ...
        self.dist_map = self._compute_distance_map(raw_grid, self.goal_xytheta[:2], resolution)
        ...
        free = (raw_grid == 0)
        self.grad_x[~free] = 0.0
        self.grad_y[~free] = 0.0
    ...
    def render(self, mode="human"):
            ...
            # 只绘制每隔 n 格的箭头
            step = max(1, int(min(H, W) / 20))  # 最多 20×20 个箭头
            i_idx = np.arange(0, H, step)
            j_idx = np.arange(0, W, step)
            Y, X = np.meshgrid((i_idx + 0.5) * self.resolution,
                            (j_idx + 0.5) * self.resolution, indexing='xy')
            U = self.grad_x[i_idx[:,None], j_idx]
            V = self.grad_y[i_idx[:,None], j_idx]
            ax.quiver(
                X, Y,
                U, V,
