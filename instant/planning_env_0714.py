...
class PathPlanningMaskEnv(gym.Env):
    ...
    def __init__(
        ...
        # --- load raw grid (row 0 is array‐top) and world_cc uses it directly ---
        map_path = get_map_file_path(scen_file)
        raw_grid, resolution = parse_map_file(map_path)  # shape (H, W)
        self.grid = raw_grid
        self.resolution = resolution
        self.world_cc = WorldCollisionChecker(self.grid, resolution)
        self.max_scan_dist = max_scan_dist

        # --- start / goal in world coords (origin bottom‐left) ---
        ...
        # --- build free mask for BFS (1=free) ---
        free = (self.grid == 0).astype(np.uint8)
        H, W = free.shape

        # --- convert goal to array coords (row counted from top) ---
        gx = int(round(self.goal_xytheta[0] / resolution))
        gy_world = self.goal_xytheta[1]
        gy = H - 1 - int(round(gy_world / resolution))

        # --- compute raw distance array (row0=top) ---
        dist_arr = self._compute_distance_map(free, np.array([gx, gy]), resolution)

        # --- fill unreachable & record max potential ---
        ...
        # --- flip to bottom‐origin so row0=world‐y=0 ---
        self.dist_map = np.flipud(dist_arr)

        # --- compute gradient on bottom‐origin dist_map ---
        ...
        # --- mask out obstacles/unreachable in gradient ---
        valid = (free == 1) & (~unreachable)
        # note unreachable was computed on dist_arr, same mask applies after flipud
    ...
    def step(self, action: int):
        ...
        dx, dy = self.state[0]-cur[0], self.state[1]-cur[1]
        dist = np.hypot(dx, dy)
        motion_dir = np.array([dx, dy], dtype=np.float32)/dist if dist>1e-8 else np.zeros(2)

        reward = self.align_coeff * float(np.dot(motion_dir, grad_vec)) + self.time_cost

        self._steps += 1
        dxg, dyg = self.state[0]-self.goal_state[0], self.state[1]-self.goal_state[1]
    ...
    def _get_obs(self) -> Dict:
        ...
        for idx, ang in enumerate(self.laser_angles):
            ...
            for s in range(max_steps):
                ...
                if not (0<=i<H and 0<=j<W) or self.grid[i, j] != 0:
    ...
    def render(self, mode="human"):
        ...
        ys = (i_idx + 0.5) * self.resolution
        xs = (j_idx + 0.5) * self.resolution
        Xs, Ys = np.meshgrid(xs, ys, indexing='xy')

        U_sub = self.grad_x[np.ix_(i_idx, j_idx)]
        V_sub = self.grad_y[np.ix_(i_idx, j_idx)]
    ...
    def _interpolate_dist(self, xy: np.ndarray) -> float:
        x, y = xy
        H, W = self.dist_map.shape
        fi = np.clip(y/self.resolution, 0, H-1-1e-6)
        fj = np.clip(x/self.resolution, 0, W-1-1e-6)
