...
class RandomResetEnv(gym.Env):
    def __init__(self, scen_files, dynamics_config, footprint_config, **env_kwargs):
        super().__init__()
        self.scen_files      = scen_files
        self.dynamics_config = dynamics_config
        self.footprint_config= footprint_config
        self.env_kwargs      = env_kwargs  # 其它比如 max_steps…
        self._make_env()
...
def make_maskable_env(scen_files, dynamics_config, footprint_config):
    base_env = RandomResetEnv(scen_files, dynamics_config, footprint_config)
...
def main():
    ...
    total_timesteps   = int(cfg.get("total_timesteps", 8000_000))
    eval_freq         = int(cfg.get("eval_freq",     800_000))
    reward_threshold  = float(cfg.get("reward_threshold",   100.0))
    n_eval_episodes   = int(cfg.get("n_eval_episodes",       5))
    n_envs = int(cfg.get("n_envs", 24))
    ...
    def make_fn(rank):
        # rank 只是区分不同进程，内部不使用
        return lambda: make_maskable_env(**env_kwargs)

    # 为 train / eval 环境各起 n_envs 个子进程
    train_env = VecMonitor(SubprocVecEnv([make_fn(i) for i in range(n_envs)]))
    eval_env  = VecMonitor(SubprocVecEnv([make_fn(i) for i in range(n_envs)]))
